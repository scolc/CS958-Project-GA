...
class UIMessageBox():
    ...
    def display_message_box(self, title, message, type):
        ...
        box_win_x = int(main_win_x + (window_width / 2 - self.m_box_width / 2))
        box_win_y = int(main_win_y + (window_height / 3 - self.m_box_height / 2))
        m_box.geometry(str(self.m_box_width) + "x" + str(self.m_box_height) +
                            "+" + str(box_win_x) + "+" + str(box_win_y))
        m_box["bg"] = self.colours.col_window_bg
        ...
        btn_y_pos = message_height + self.y_pad * 2
        ...
        if type == "ok":
            ...
            return self.display_message_difficulty(m_box, btn_y_pos)
        else:
            return False
    ...
    def display_message_ok(self, m_box : Toplevel, y_pos):
        ...
        btn = Button(btn_frame,
                        text = "Ok",
                        font = self.m_box_font,
                        command = lambda: self.message_box_choice(m_box, True, user_confirm))
    ...
    def display_message_yes_no(self, m_box : Toplevel, y_pos):
        ...
        no_btn = Button(no_btn_frame,
                        text = "No",
                        font = self.m_box_font,
                        command = lambda : self.message_box_choice(m_box, False, user_confirm))
        
        btn_width = self.tile_size * 3
        yes_x_pos = self.m_box_width / 2 - btn_width - self.x_pad / 2
        
        yes_btn_frame.place(x = yes_x_pos - self.m_box_frame_pad,
                        y = y_pos - self.m_box_frame_pad,
                        width = btn_width + self.m_box_frame_pad * 2,
                        height = self.tile_size + self.m_box_frame_pad * 2)
        ...
        no_btn_frame.place(x = no_x_pos - self.m_box_frame_pad,
                            y = y_pos - self.m_box_frame_pad,
                            width = btn_width + self.m_box_frame_pad * 2,
                            height = self.tile_size + self.m_box_frame_pad * 2)
    ...
    def display_message_difficulty(self, m_box : Toplevel, y_pos):
        """
        A function to display three difficulty buttons on the message box.
        Returns the choice depending on the button pressed and returns to
        the main window.
        """
        # Display the buttons
        user_confirm = StringVar() # Updated depending on which button pressed
        user_confirm.set("")

        med_btn_frame = Frame(m_box,
                        bg = self.colours.col_bg_en_norm,
                        highlightthickness = 1,
                        highlightbackground = self.colours.col_fg_en_norm)
        med_btn = Button(med_btn_frame,
                        text = "Medium",
                        font = self.m_box_font,
                        command = lambda: self.message_box_choice(m_box, "medium", user_confirm))
        
        easy_btn_frame = Frame(m_box,
                        bg = self.colours.col_bg_en_norm,
                        highlightthickness = 1,
                        highlightbackground = self.colours.col_fg_en_norm)
        easy_btn = Button(easy_btn_frame,
                        text = "Easy",
                        font = self.m_box_font,
                        command = lambda : self.message_box_choice(m_box, "easy", user_confirm))
        
        hard_btn_frame = Frame(m_box,
                        bg = self.colours.col_bg_en_norm,
                        highlightthickness = 1,
                        highlightbackground = self.colours.col_fg_en_norm)
        hard_btn = Button(hard_btn_frame,
                        text = "Hard",
                        font = self.m_box_font,
                        command = lambda : self.message_box_choice(m_box, "hard", user_confirm))
        
        btn_width = self.tile_size * 2
        med_x_pos = self.m_box_width / 2 - btn_width / 2
        med_btn_frame.place(x = med_x_pos - self.m_box_frame_pad,
                        y = y_pos - self.m_box_frame_pad,
                        width = btn_width + self.m_box_frame_pad * 2,
                        height = self.tile_size + self.m_box_frame_pad * 2)
        med_btn.place(x = self.m_box_frame_pad - med_btn_frame["highlightthickness"],
                    y = self.m_box_frame_pad - med_btn_frame["highlightthickness"],
                    width = btn_width,
                    height = self.tile_size)
        
        yes_x_pos = med_x_pos - btn_width - self.x_pad
        
        easy_btn_frame.place(x = yes_x_pos - self.m_box_frame_pad,
                        y = y_pos - self.m_box_frame_pad,
                        width = btn_width + self.m_box_frame_pad * 2,
                        height = self.tile_size + self.m_box_frame_pad * 2)
        easy_btn.place(x = self.m_box_frame_pad - easy_btn_frame["highlightthickness"],
                    y = self.m_box_frame_pad - easy_btn_frame["highlightthickness"],
                    width = btn_width,
                    height = self.tile_size)

        no_x_pos = med_x_pos + btn_width + self.x_pad
        
        hard_btn_frame.place(x = no_x_pos - self.m_box_frame_pad,
                            y = y_pos - self.m_box_frame_pad,
                            width = btn_width + self.m_box_frame_pad * 2,
                            height = self.tile_size + self.m_box_frame_pad * 2)
        hard_btn.place(x = self.m_box_frame_pad - hard_btn_frame["highlightthickness"],
                        y = self.m_box_frame_pad - hard_btn_frame["highlightthickness"],
                        width = btn_width,
                        height = self.tile_size)
